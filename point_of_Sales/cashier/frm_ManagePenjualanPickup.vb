...
Public Class frm_ManagePenjualanPickup
    ...
    Private totalPenjualanPickup As Decimal = 0
    ...
    Sub hitungSubtotal()
        ...
        harga = BersihkanAngka(txt_hrgaPickup.Text)
        Decimal.TryParse(txt_qtyPickup.Text, qty)

        Dim subtotal As Decimal = harga * qty

        'Format textbox
        txt_hrgaPickup.Text = harga.ToString("N0")
        txt_subTotalPickup.Text = subtotal.ToString("N0")
    End Sub
    ...
    Sub load_ListPenjualanPending()
        ...
        Try
            ...
            "SELECT no_penjualan, jenis_penjualan, tanggal, total_penjualan, dana_diterima, kembalian, metode_pembayaran, catatan, atas_nama, kontak_penerima, status_pengiriman
             FROM tblpenjualan 
             WHERE status_penjualan = 'Pending' AND jenis_penjualan = 'Pickup'
        ...
        End Try
    End Sub
    Sub load_ListPenjualanPickupPacking()
        dgv_penjualanPackingPickup.Rows.Clear()
            ...
            Using cmd As New MySqlCommand(query, conn)
                Using dr As MySqlDataReader = cmd.ExecuteReader()
                    While dr.Read()
                        ...
                        dgv_penjualanPackingPickup.Rows.Add(
                        dgv_penjualanPackingPickup.Rows.Count + 1,
                        dr("no_penjualan").ToString(),
                        dr("jenis_penjualan").ToString(),
                        tgl.ToString("dd/MM/yyyy"),
                        "Rp. " & Format(CDec(dr("total_penjualan")), "N0"),
        ...
        Catch ex As Exception
            MsgBox("Gagal memuat data penjualan packing: " & ex.Message, vbExclamation)
        Finally
            conn.Close()
        End Try
    End Sub
    ...
    Private Sub frm_ManagePenjualanPickup_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        dgv_listPenjualanPending.RowTemplate.Height = 30

        dbconn()
        GenerateNoPenjualan()
        load_ListPenjualanPending()
        load_ListPenjualanPickupPacking()
        load_barang()
        cbo_statusPickup.Text = "Pending"
    End Sub

    Private Sub txt_noPenjualan_TextChanged(sender As Object, e As EventArgs) Handles txt_noPenjualanPickup.TextChanged
    ...
    Private Sub cbo_brg_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbo_brgPickup.SelectedIndexChanged
    ...
    Private Sub dgv_detailPenjualan_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgv_detailPenjualanPickup.CellDoubleClick
        ...
        If MessageBox.Show("Hapus item ini?", "Hapus", MessageBoxButtons.YesNo) = DialogResult.Yes Then
            ...
            lbl_grandTotalPickup.Text = hitungTotalPenjualan().ToString("N0")
            lbl_totalQtyPickup.Text = "Total Barang: " & HitungTotalQty().ToString()
            lbl_QtyPickup.Text = "00"
            lbl_hargaSatuanPickup.Text = "00.00"
            lbl_subTotalPickup.Text = "00.00"
    ...
    Private Sub btn_tmbhDetBeli_Click(sender As Object, e As EventArgs) Handles btn_tmbhDetBeliPickup.Click
        ' VALIDASI INPUT
    ...
    Private Sub txt_qty_TextChanged(sender As Object, e As EventArgs) Handles txt_qtyPickup.TextChanged
        hitungSubtotal()
    End Sub

    Private Sub cbo_statusPenjualan_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbo_statusPenjualanPickup.SelectedIndexChanged
    ...
    Private Sub txt_danaDiterima_TextChanged(sender As Object, e As EventArgs) Handles txt_danaDiterimaPickup.TextChanged
        Dim dana As Decimal
    ...
    Private Sub txt_danaDiterima_Leave(sender As Object, e As EventArgs) Handles txt_danaDiterimaPickup.Leave
    ...
    Private Sub txt_danaDiterima_Enter(sender As Object, e As EventArgs) Handles txt_danaDiterimaPickup.Enter
        txt_danaDiterimaPickup.Text = txt_danaDiterimaPickup.Text.Replace(".", "").Replace(",", "")
    End Sub

    Private Sub btnCancel_Click(sender As Object, e As EventArgs) Handles btnCancelPickup.Click
        clear()
    End Sub

    Private Sub btn_simpanPenjualan_Click(sender As Object, e As EventArgs) Handles btn_simpanPenjualanPickup.Click
    ...
    Sub changelongpaper()
        ...
        longpaper = 0
        rowcount = dgv_detailPenjualanPickup.Rows.Count
    ...
    Private Sub PD_PrintPage(sender As Object, e As Printing.PrintPageEventArgs) Handles PD.PrintPage
        ...
        y += 20
        ' Info Transaksi
        e.Graphics.DrawString("No: " & txt_noPenjualanPickup.Text, f10, Brushes.Black, leftmargin, y)
        y += 18
        e.Graphics.DrawString("Tanggal: " & dtp_tglJualPickup.Value.ToString("dd/MM/yyyy HH:mm"), f10, Brushes.Black, leftmargin, y)
        y += 18
    ...
    Private Sub txtCariPenjualanPacking_TextChanged(sender As Object, e As EventArgs) Handles txtCariPenjualanPacking.TextChanged
        dgv_penjualanPackingPickup.Rows.Clear()
        Try
            conn.Open()
            cmd = New MySqlCommand("
                ...
                ", conn)
            cmd.Parameters.AddWithValue("@keyword", "%" & txtCariPenjualanPacking.Text & "%")
            dr = cmd.ExecuteReader
            While dr.Read
                Dim tgl As Date = Date.Parse(dr("tanggal").ToString())
                dgv_listPenjualanPending.Rows.Add(
    ...
    Private Sub dgv_penjualanPackingPickup_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgv_penjualanPackingPickup.CellClick
        If e.RowIndex < 0 Then Exit Sub

        Dim idJualPacking As String = dgv_penjualanPackingPickup.Rows(e.RowIndex).Cells("colNoPenjualanPickupPacking").Value.ToString()
